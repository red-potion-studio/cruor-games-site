...
// --- TAB SELECTOR ---
const tabs = data.variants.map((v, i) => {
...
const variantsHTML = data.variants.map((v, i) => {
  ...
  const weakHTML = renderSimpleSection("Weak Spots", "monster-weakspots", data.weakSpots);
  const affixHTML = renderSimpleSection("Affixes", "monster-affixes", data.affixes);
  ...
  <article class="content-article terrifying-monster">
    ...
    <p class="meta">Written by ${data.author || "Unknown"} — ${data.date || ""}</p>
  </article>
  `;
}
...
function renderVariant(v) {
  ...
  const traitsHTML = renderSimpleSection("Traits", "monster-traits", v.traits, true);
...
function renderLair(data) {
  ...
  <section class="collapsible monster-lair">
    ...
    ${lair.list ? lair.list.map(a => `<p><b>${a.name}.</b> ${a.text}</p>`).join("") : ""}
  </section>`;
}
